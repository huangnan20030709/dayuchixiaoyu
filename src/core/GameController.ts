...
export class GameController extends Container {
  ...
  update = ({ deltaMS }: Ticker) => {
    ...
    let plusSpeed = 0;
    if (this.playerFish) {
      plusSpeed = Math.floor(this.playerFish.level / 50) / 2;
    }
    ...
    if (isHit && litterFish && this.playerFish) {
  ...
  checkIsHit(): { isHit: boolean; litterFish: LitterFish | null } {
    for (let i = 0; i < this.litterFishArr.length; i++) {
      ...
      if (this.playerFish == null) return { isHit: false, litterFish: null };
      ...
      // 玩家鱼和小鱼都是从左向右移动
      if (
        this.playerFish?.direction === 0 &&
        litterFish.direction === 0 &&
        playerBounds.x < litterBounds.x &&
        playerBounds.x + playerBounds.width > litterBounds.x
      ) {
      ...
      // 玩家鱼和小鱼都是从右向左移动
      if (
        this.playerFish?.direction === 1 &&
        litterFish.direction === 1 &&
        playerBounds.x > litterBounds.x &&
        playerBounds.x < litterBounds.x + litterBounds.width
      ) {
        if (checkTopOrDownHit()) {
          if (this.playerFish?.level == litterFish.level) {
            continue;
  ...
  reStart() {
    ...
    this.playerFish && this.removeChild(this.playerFish);
